...
// the tests below are based on orignal tests ("index.test.js").
/*
...
Deno.test("文字列には干渉しない", async t => {
  const vs = [
    { type: `文字列はエスケープしつつそのまま`, input: '{"{s:0}}', expected: `"{\\"{s:0}}"`},
    { type: "リストの中身の文字列もそのまま", input: ['{"{s:0}}'], expected:`["{\\"{s:0}}"]`},
    { type: "オブジェクトのキーの文字列もそのまま",  input: { '{"{s:0}}': 1 }, expected: `{"{\\"{s:0}}": 1}`}
  ]
  
  await vs.reduce((pre, {type, input, expected}) => pre.then( async () => {
    await t.step(`OK: ${type}`, () => {
      assertEquals(stringify(input), expected)
    })
  }), Promise.resolve())
})


Deno.test("1行の許容文字数 max の値に応じてインデントを変更", async t => {
  const obj = { bool: true, array: [1, 2, 3], null: null }
  const one_line = `{"bool": true, "array": [1, 2, 3], "null": null}`
  const obj_break = `{\n  "bool": true,\n  "array": [1, 2, 3],\n  "null": null\n}`
  const array_break = `{\n  "bool": true,\n  "array": [\n    1,\n    2,\n    3\n  ],\n  "null": null\n}`

  await t.step("JSON.stringify(data) の結果が max 以下なら1行で出力", () => {
    assertEquals(stringify(obj), one_line)
  })

  await t.step("max を小さくすると top level の要素で break", () => {
    assertEquals(stringify(obj, undefined, 30), obj_break)
  })

  await t.step("max をさらに小さくすると nest の中身でも break", () => {
    assertEquals(stringify(obj, undefined, 15), array_break)
  })
})
